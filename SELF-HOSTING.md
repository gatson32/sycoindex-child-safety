...
- **Write endpoints that hit external services.** `/api/submit` and the waitlist/keys endpoints accept requests and return 200, but the outbound email side of the pipeline is not included — wire up your own SMTP or Resend integration if you need delivery.
